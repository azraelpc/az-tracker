...
Modplayer.prototype.mix = function(ape) {
  ...
  if (mod.player && mod.delayfirst==0) {
    ...
    // update this.chvu from player channel vu (con blindaje de seguridad)
    if (mod.player && mod.player.chvu && mod.chvu) {
      for(var i=0; i < Math.min(mod.player.channels, mod.player.chvu.length, mod.chvu.length); i++) {

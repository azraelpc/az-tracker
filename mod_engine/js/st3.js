  ...
  for(var s=0;s<buflen;s++)
  {
    ...
    // add channel output to left/right master outputs
      ...
      if (mod.channel[ch].noteon || (!mod.channel[ch].noteon && mod.channel[ch].volramp<1.0)) {
        // Validación completa y robusta de la muestra y sus datos
        if (si >= 0 && mod.sample && mod.sample[si] && 
            mod.sample[si].data && typeof mod.sample[si].data.length === 'number' && 
            Math.floor(mod.channel[ch].samplepos) < mod.sample[si].data.length && 
            mod.sample[si].length > mod.channel[ch].samplepos) {
